...
#define COMP_DEBUGINFO(format,...) virtual std::string rtti_string() \
{\
	char s[512]; \
...
namespace hs 
{
	...
	public:
		...
		template <typename Type> Type *add();
...
//		void get_entities_possesing_components(std::vector<entity*> &result, ...);
		...
		template <typename COMP_TYPE> COMP_TYPE *add_component(entity *e, COMP_TYPE *c);
		template <typename COMP_TYPE> COMP_TYPE *add_component(entity *e, COMP_TYPE *c, uid fam_id);		
		
		template<typename T> void remove_component(entity *e);
		template<typename COMP_TYPE> void remove_component(entity *e, COMP_TYPE *c);
		template<typename COMP_TYPE> void remove_component(entity *e, COMP_TYPE *c, uid family_id);
	...
	template <typename T>
	T *entity_manager::add_component(entity *e)
	{
		T *comp = new T();
	...
	COMP_TYPE *entity_manager::add_component(entity *e, COMP_TYPE *c)
	{
	...
	template <typename COMP_TYPE>
	COMP_TYPE *entity_manager::add_component(entity *e, COMP_TYPE *c, uid fam_id)
	{
	...
	template <typename T> 
	T *entity_manager::get_component(entity *e)
	{
	...
	template<typename T>
	void entity_manager::remove_component(entity *e)
	{
	...
	template<typename COMP_TYPE>
	void entity_manager::remove_component(entity *e, COMP_TYPE *c)
	{
	...
	template<typename COMP_TYPE>
	void entity_manager::remove_component(entity *e, COMP_TYPE *c, uid family_id)
	{
