	...
	)
{
	...
	if( (x_pos_file == NULL) ||
		...
		(z_neg_file == NULL) )
	{
		result_string_pointer = (char*)"Invalid cube map files list";
		return 0;
	}
	/*	capability checking	*/
	if( query_cubemap_capability() != SOIL_CAPABILITY_PRESENT )
	{
		result_string_pointer = (char*)"No cube map capability present";
		return 0;
	}
	...
	)
{
	...
	if( (x_pos_buffer == NULL) ||
		...
		(z_neg_buffer == NULL) )
	{
		result_string_pointer = (char*)"Invalid cube map buffers list";
		return 0;
	}
	/*	capability checking	*/
	if( query_cubemap_capability() != SOIL_CAPABILITY_PRESENT )
	...
	)
{
	...
	if( filename == NULL )
	{
		result_string_pointer = (char*)"Invalid single cube map file name";
		return 0;
	}
	/*	does the user want direct uploading of the image as a DDS file?	*/
	if( flags & SOIL_FLAG_DDS_LOAD_DIRECT )
	...
	for( i = 0; i < 6; ++i )
	{
		if( (face_order[i] != 'N') &&
			...
			(face_order[i] != 'D') )
		{
			result_string_pointer = (char*)"Invalid single cube map face order";
			return 0;
		};
	}
	/*	capability checking	*/
	...
	{
		SOIL_free_image_data( img );
		result_string_pointer = (char*)"Single cubemap image must have a 6:1 ratio";
		return 0;
	}
	...
	)
{
	...
	if( buffer == NULL )
	{
		result_string_pointer = (char*)"Invalid single cube map buffer";
		return 0;
	}
	/*	does the user want direct uploading of the image as a DDS file?	*/
	if( flags & SOIL_FLAG_DDS_LOAD_DIRECT )
	...
	)
{
	...
	if( data == NULL )
	{
		result_string_pointer = (char*)"Invalid single cube map image data";
		return 0;
	}
	/*	face order checking	*/
	for( i = 0; i < 6; ++i )
	...
	)
{
	...
	if( tex_id )
	{
		...
		hs::texture2d::bound_texture = tex_id;
	...
	)
{
	...
	{
		result_string_pointer = (char*)"Image loaded";
	}
	return result;
}
		...
		int loading_as_cubemap )
{
	...
	if( cubemap )
	{
		...
		{
			/*	we can't do it!	*/
			result_string_pointer = (char*)"DDS image was a cubemap";
			return 0;
		}
		...
		{
			/*	we can't do it!	*/
			result_string_pointer = (char*)"Direct upload of cubemap images not supported by the OpenGL driver";
			return 0;
		}
	...
	} else
	{
		...
		{
			/*	we can't do it!	*/
			result_string_pointer = (char*)"DDS image was not a cubemap";
			return 0;
		}
